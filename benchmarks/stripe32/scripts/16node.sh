...
#SBATCH -e 16node.err # file to save job's STDERR (%j = JobId)
#SBATCH -o 16node.out # file to save job's STDOUT (%j = JobId)
#SBATCH --export=ALL
...
testdir=/oasis/scratch/comet/edisj/temp_project/benchmarking32/$SLURM_JOB_ID
mkdir -p $testdir
...
for j in 1 2 3
do
    echo "Running benchmark for 16 nodes, 384 processes"
    time ibrun -np 384 python -W ignore /oasis/projects/nsf/azs119/edisj/Comet/benchmarks/stripe32/scripts/benchmark.py $testdir/YiiP_system.pdb $testdir/YiiP_system_9ns_center100x.h5md $1/16node_$j
done
