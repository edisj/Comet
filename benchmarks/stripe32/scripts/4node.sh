...
#SBATCH -e 4node.err # file to save job's STDERR (%j = JobId)
#SBATCH -o 4node.out # file to save job's STDOUT (%j = JobId)
#SBATCH --export=ALL
...
for j in 1 2 3
do
    echo "Running benchmark for 4 nodes, 96 processes"
    time ibrun -np 96 python -W ignore /oasis/projects/nsf/azs119/edisj/Comet/benchmarks/stripe32/scripts/benchmark.py $testdir/YiiP_system.pdb $testdir/YiiP_system_9ns_center100x.h5md $1/4node_$j
done
